...
4. **統一認證機制**：同一組帳號密碼用於 Registry 登入、UI 存取和 Traefik Dashboard。
...
**此認證資訊將用於 Registry 登入、UI 存取和 Traefik Dashboard 的統一認證。**
...
執行 `start.sh` 時，系統會以對話方式詢問你所有必要參數（如 Registry 網域、UI 網域等），以及 htpasswd 認證資訊。
...
- UI 入口：`https://ui.your-domain.com`，可用瀏覽器開啟 Registry UI，使用您設定的 htpasswd 認證登入。
- Traefik Dashboard：`https://traefik.your-domain.com`，使用相同的 htpasswd 認證登入。
- 推送/拉取映像檔：
  ```bash
...
1. **準備 htpasswd 認證資訊**
    ...
    **若已經有在執行中的服務，想要更新 htpasswd，可以直接編輯 ./registry/auth/htpasswd 檔案和 ./traefik/config/middlewares.yml 檔案，然後執行下列指令重啟服務，讓新密碼生效：**
    ```bash
